...
# Configuração de logs para simular o envio [cite: 85]
...
def get_db_connection():
    # Obtém a URL do banco do ambiente (definida no docker-compose) [cite: 69]
    return psycopg2.connect(os.getenv("DATABASE_URL"))

@app.get("/notifications/pending")
def get_pending():
    """Consulta usuários com notificationEnabled = true [cite: 42]"""
...
def send_notifications():
    """Simula o envio de notificações e retorna o total [cite: 43, 44]"""
    conn = get_db_connection()
    cur = conn.cursor()
    ...
    total = 0
    for user in users:
        # Simulação via log no console [cite: 43]
